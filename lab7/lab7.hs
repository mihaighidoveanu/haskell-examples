...
isBloodOrange x = case x of Orange variety _ -> variety `elem` tr
                            _ -> False


-- 2.
bloodOrangeSegments :: [Fruit] -> Int
--bloodOrangeSegments frs = sum [(\(Orange _ segm) -> segm) fruit | fruit <- frs, isBloodOrange fruit ]
bloodOrangeSegments frs = sum[segm | x@(Orange _ segm) <- frs]

-- 3.
worms :: [Fruit] -> Int
worms frs = sum[1 | x@(Apple _ True) <- frs]
...
prop_taut2 p = not (tautology (Not p)) || not (satisfiable  p)
