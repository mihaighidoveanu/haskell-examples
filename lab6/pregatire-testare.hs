...
lungimePlus n 0 = n
lungimePlus n l 
 | l < 0 = error "lungimea nu poate fi negativa"
 | otherwise = 0 : lungimePlus n (l-1)
...
normalizeazaLungime (nx, ny)
 | lx < ly = (lungimePlus nx (ly - lx), ny)
 | otherwise = (nx, lungimePlus ny (lx - ly))
 where 
 	lx = length nx
 	ly = length ny  
...
lteN (c1 : n1) (c2 : n2) 
 | c1 < c2 = True 
 | c1 > c2 = False
 | otherwise =  lteN n1 n2


-- 2b (0,25p).  Scrieti o functie care ia doua Numere ca argumente
-- si verifica daca primul Numar este mai mic sau egal cu al doilea
lte :: Numar -> Numar -> Bool
lte nx ny = lteN normX normY
	where (normX, normY) = normalizeazaLungime (nx, ny)
...
f i (c,nr) = (cat, rest : nr)
	where 
		cat = (i + c) `div` 10
		rest = (i + c) `mod` 10

propagaFold' :: [Int] -> (Cifra, [Cifra])
propagaFold' xs =  foldr f (0,[]) xs
...
propagaFold xs 
 | cat == 0 =  numarFaraCat
 | otherwise = cat : numarFaraCat
 where (cat, numarFaraCat) = propagaFold' xs
...
plusLista [] [] = []
plusLista (c1 : n1) (c2 : n2) = (c1 + c2) : (plusLista n1 n2)
...
plus nx ny = propagaFold (plusLista nx' ny')
	where (nx', ny') = normalizeazaLungime (nx, ny) 
...
minusLista n1 n2 = [c1 - c2 | (c1,c2) <- zip n1 n2 ]
...
minus n1 n2
 | n2 `lte` n1 =propagaFold (minusLista n1' n2')
...
mul [] l = []
mul n1 n2 = plus (mulC (last n1) n2) (mul (init n1) (mul10 n2)) 
