...
lungimePlus l 0 = l
lungimePlus l n = 0:lungimePlus l (n-1)
...
normalizeazaLungime (p1,p2) =  if length p1 > length p2 then (p1,(lungimePlus p2 (length p1 - length p2))) else ((lungimePlus p1 (length p2 - length p1)), p2)
...
lteN [] [] = True
lteN (c1:p1) (c2:p2) = if c1 < c2 then True else if c1 > c2 then False else lteN p1 p2

-- 2b (0,25p).  Scrieti o functie care ia doua Numere ca argumente
-- si verifica daca primul Numar este mai mic sau egal cu al doilea
lte :: Numar -> Numar -> Bool
lte p1 p2 = lteN (fst (normalizeazaLungime (p1,p2))) (snd (normalizeazaLungime (p1,p2)))
...
f :: Int -> (Cifra, Numar) -> (Cifra, Numar)
f n (c,nr) = ( ((n+c) `div` 10), ((n+c) `mod` 10):nr )

propagaFold' :: [Int] -> (Cifra, [Cifra])
propagaFold' l = foldr f (0,[]) l
...
propagaFold l = fst (propagaFold' l) : snd (propagaFold' l)
...
plusLista l [] = l
plusLista (p1:c1) (p2:c2) = p1+p2 : plusLista c1 c2 
...
plus n1 n2 = propagaFold (plusLista n1' n2')
	where (n1',n2') = normalizeazaLungime (n1,n2)
...
minusLista c1 c2 = [(fst p) - (snd p)|p<-zip c1 c2]
...
minus n1 n2 = if (lte n2 n1) then propagaFold (minusLista n1 n2) else error "Numere negative neimplementate"
...
mul [] l = []
mul n1 n2 = plus (propagaFold (mulC (last n1) n2)) (mul (init n1) (mul10 n2)) 
